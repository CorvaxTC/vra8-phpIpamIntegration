...
def do_allocate_ip(self, auth_credentials, cert):
    ...
    try:
        ...
        for allocation in self.inputs["ipAllocations"]:
            ...
            allocation_result.append(allocate(resource, allocation, self.context,baseUrl,headers,cert))
    except Exception as e:
        try:
            rollback(allocation_result,baseUrl,headers,cert)
...
def allocate(resource, allocation, context, baseUrl,headers,cert):
    ...
    for range_id in allocation["ipRangeIds"]:
        ...
        try:
            return allocate_in_range(range_id, resource, allocation, context, baseUrl,headers,cert)
        except Exception as e:
            last_error = e
...
def allocate_in_range(range_id, resource, allocation, context, baseUrl,headers,cert):

    ## Plug your implementation here to allocate an ip address
    ## ...
    ## Allocation successful

    #https://{{ipamFQDN}}/api/{{appName}}/addresses/first_free/{{subnetId}}/?hostname={{hostname}}&description={{hostDescription}}&owner={{owner}}

    requestUrl = baseUrl + "/addresses/first_free/" + range_id
    
    #logging.info(resource)

    if resource["name"] is not None:
      logging.info("IP Allocation hostname '" + str(resource["name"]) + "'")
      requestUrl = requestUrl + "/?hostname=" + str(resource["name"])
      
      if resource["owner"] is not None:
        logging.info("Adding owner '" + str(resource["owner"]) +"'")
        requestUrl = requestUrl + "&owner=" + str(resource["owner"])
      
      #if resource["description"] is not None:
       #logging.info(f"Adding description '{str(resource["description"])}'")
       #requestUrl = requestUrl + "&description=" + str(resource["description"])

      logging.info(f"Request url = '{requestUrl}'")
      logging.info("Retrieving IP from php Ipam")

      response = requests.post(requestUrl,verify=cert,headers=headers)

      if response.status_code == 201:
        body = response.json()

        ipAddresses = body["data"]
        ipam_address_id =  body["id"]

        logging.info(f"Successfully retrieved ip '{ipAddresses}'")
      ...
      logging.error("missing required hostname")
      raise "Unable to assign ip. Missing required hostname"
...
def rollback(allocation_result,baseUrl,headers,cert):
    for allocation in reversed(allocation_result):
      ...
      subnetId = allocation.get("ipRangeId", None)

      # release the address
      response = requests.delete(baseUrl + "addresses/" + ipAddresses + "/" + subnetId,verify=cert,headers=headers)

      if response.status_code == 200:
        logging.info(f"Successfull rollback of IP '{ipAddresses}' from subnet '{subnetId}'")      
      elif response.status_code == 401:
        logging.error(f"Invalid credentials error: {str(response.content)}")
        raise Exception(f"Invalid credentials error: {str(response.content)}")
      else:
